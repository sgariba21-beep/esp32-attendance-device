...
TaskHandle_t hFingerprint = NULL, hNetwork = NULL, hEnrollment = NULL;
...
// Stored credentials (loaded from SPIFFS at boot)
String wifiSSID = "";
String wifiPASS = "";
...
<title>Attendance Device - WiFi Setup</title>
<style>
...
<div class="label">WiFi Name (SSID)</div>
...
void startCaptivePortal() {
  esp_task_wdt_delete(NULL); // remove current task from watchdog monitoring
  ...
  portalServer.on("/save", []() {
    ...
    portalServer.send(200, "text/plain", "Saved! Device will restart and connect now...");
    delay(1500);
    ESP.restart();
  });

  // Redirect all unknown paths (catches iOS/Android captive portal probes)
  ...
  portalServer.begin();
  Serial.println("Portal server started. Waiting for WiFi credentials...");

  // Breathe purple while waiting — blocks here until user submits and device restarts
  while (true) {
    dnsServer.processNextRequest();
    portalServer.handleClient();
    // Use a simple LED toggle since breathing needs delay loops
...
bool postJSONToUrl(const String &jsonPayload, const char* targetUrl, int &outHttpCode, String &outBody) {
  ...
  sharedHttp.setFollowRedirects(HTTPC_STRICT_FOLLOW_REDIRECTS);
  sharedHttp.setTimeout(20000);

  bool result = false;
  for (int attempt = 1; attempt <= POST_MAX_RETRIES; ++attempt) {
    if (!sharedHttp.begin(sharedClient, targetUrl)) {
      outHttpCode = -2; outBody = "HTTP begin failed";
      Serial.println("HTTP begin failed");
      break;
    }
    sharedHttp.addHeader("Content-Type", "application/json");
    int code = sharedHttp.POST(jsonPayload);
    outHttpCode = code;
    if (code > 0) {
      outBody = sharedHttp.getString();
      Serial.printf("POST -> code=%d body=%s\n", code, outBody.c_str());
      sharedHttp.end();
      if ((code >= 200 && code < 300) || code == 400) { result = true; break; }
      ...
      outBody = sharedHttp.errorToString(code);
...
void reportEnrollUpdate(int row, const String &status, int fingerId, const String &note) {
  ...
  sharedHttp.setFollowRedirects(HTTPC_STRICT_FOLLOW_REDIRECTS);
  sharedHttp.setTimeout(20000);
  if (sharedHttp.begin(sharedClient, url)) {
    int code = sharedHttp.GET();
    String body = (code > 0) ? sharedHttp.getString() : sharedHttp.errorToString(code);
...
void EnrollmentTask(void *pvParameters) {
  ...
  for (;;) {
    ...
    if (WiFi.status() == WL_CONNECTED) {
      ...
      if (sharedHttp.begin(sharedClient, url)) {
        code = sharedHttp.GET();
        if (code > 0) body = sharedHttp.getString();
        sharedHttp.end();
      }
      xSemaphoreGive(httpMutex);
...
void FingerprintTask(void *pvParameters) {
  ...
  for (;;) {
    ...
    // Normal scan behavior — skip scanning if an enrollment job is waiting
    if (enrollmentJobPending) {
    ...
    if (fid > 0) {
      ...
      if (role == "master") {
        // Master scanned: launch captive portal so user can enter new WiFi credentials
        Serial.println("Master scanned: launching WiFi captive portal.");
        // Blink yellow 5 times to signal portal is starting
        for (int i = 0; i < 5; i++) {
          setSensorLED(FINGERPRINT_LED_ON, 0, FINGERPRINT_LED_YELLOW);
        ...
        startCaptivePortal(); // blocks until new creds saved and device restarts
      } else {
...
void setup() {
  ...
  // WiFi connect — load credentials from SPIFFS, fall back to captive portal if missing/failed
  bool hasCreds = loadWiFiCreds();
  if (!hasCreds) {
    Serial.println("No WiFi credentials found. Launching captive portal...");
    setupFingerprint(); // sensor must be ready so portal can blink the LED
    startCaptivePortal(); // blocks; restarts when creds saved
  }
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    Serial.println("\nWiFi failed with saved credentials — continuing in offline mode.");
    Serial.println("Scan master fingerprint to reconfigure WiFi.");
    setupFingerprint();
    startCaptivePortal();
  }
  ...
  BaseType_t ok3 = xTaskCreatePinnedToCore(EnrollmentTask, "EnrollmentTask", 8192, NULL, 1, &hEnrollment, 0);
