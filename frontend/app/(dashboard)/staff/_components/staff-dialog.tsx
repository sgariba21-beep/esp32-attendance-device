...
type Labels = { label_member: string; label_unit: string }
...
function FingerEnrollRow({ label, slot, memberId, deviceId, defaultFid }: {
  label: string; slot: 'fin1' | 'fin2'; memberId: string; deviceId: string; defaultFid: number
}) {
  ...
  if (done) return (
    <Alert variant="success">
      ...
      <AlertDescription>{label} — job queued (sensor slot {fid}). Tell the member to scan when prompted.</AlertDescription>
    </Alert>
  )
  ...
  return (
    <div className="space-y-2">
      {!open ? (
        <Button variant="outline" size="sm" onClick={() => setOpen(true)}>+ Enroll {label}</Button>
      ) : (
        <div className="flex items-center gap-2">
          <span className="text-sm w-20 shrink-0">{label}:</span>
          <Input type="number" min={1} max={127} value={fid} onChange={(e) => setFid(e.target.value)} className="h-8 w-24" placeholder="Slot" />
...
function FingerEditRow({ label, slot, fid, memberId, deviceId, defaultFid }: {
  label: string; slot: 'fin1' | 'fin2'; fid: number; memberId: string; deviceId: string; defaultFid: number
}) {
  ...
  if (fid && !deleted) return (
    <div className="space-y-1">
      <div className="flex items-center gap-2">
        ...
        <Button type="button" variant="ghost" size="sm" onClick={handleDelete} disabled={deleting} className="h-7 px-2 text-xs text-destructive hover:text-destructive ml-auto">
  ...
  return <FingerEnrollRow label={label} slot={slot} memberId={memberId} deviceId={deviceId} defaultFid={defaultFid} />
}

type EnrollStep = { memberId: string; deviceId: string; fullname: string; deviceName: string }

const emptyForm = { sid: '', fullname: '', device_id: '', institution_id: '' }

export function StaffDialog({ open, onOpenChange, member, devices, usedFids, labels, institutions, isPlatformAdmin }: Props) {
  ...
  useEffect(() => {
    if (open) {
      setError(null); setEnrollStep(null)
      setForm(member
        ? { sid: member.sid, fullname: member.fullname, device_id: member.device_id ?? '', institution_id: member.institution_id ?? '' }
        : { ...emptyForm, device_id: isPlatformAdmin ? '' : (devices[0]?.id ?? '') }
      )
    }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (member) {
      const result = await updateStaffMember(member.id, { sid: form.sid, fullname: form.fullname, device_id: form.device_id, fin1: member.fin1, fin2: member.fin2 })
      setLoading(false)
      if (result.error) { setError(result.error); return }
      onOpenChange(false)
    } else {
      const result = await createStaffMember({ sid: form.sid, fullname: form.fullname, device_id: form.device_id, fin1: 0, fin2: 0 })
      setLoading(false)
      if (result.error) { setError(result.error); return }
      const device = devices.find((d) => d.id === form.device_id)
      setEnrollStep({ memberId: result.id!, deviceId: form.device_id, fullname: form.fullname, deviceName: device ? `${device.group_name} ${device.unit_name}` : '' })
    }
  }
    ...
    return (
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader><DialogTitle>{labels.label_member} added</DialogTitle></DialogHeader>
      ...
      <DialogContent className="sm:max-w-md">
        ...
        <form onSubmit={handleSubmit} className="space-y-4 py-2">
          <div className="space-y-2">
            ...
            <Input id="fullname" value={form.fullname} onChange={(e) => set('fullname', e.target.value)} placeholder="e.g. Jane Doe" required />
          </div>

          <div className="space-y-2">
            <Label htmlFor="sid">ID</Label>
            <Input id="sid" value={form.sid} onChange={(e) => set('sid', e.target.value)} placeholder="e.g. T001" required />
          </div>
